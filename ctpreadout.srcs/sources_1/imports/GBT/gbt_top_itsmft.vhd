...
entity gbt_top is
    ...
    gbt_data_user_o                              : out gbt_reg84_A(1 to NUM_LINKS);
    --  Comment:  Must be sync with TTC_PON data.         
    GBT_TX_CLK240_i                             : in  std_logic; 
    GBT_TX_CLK240_EN_i                          : in  std_logic_vector (1 to NUM_LINKS);   -- Comment. Tick, sync w.r.t  TTC_PON CLOCK 
    GBT_TX_WORDCLK_o                            : out std_logic_vector (1 to NUM_LINKS);
    --                 
    GBT_RX_CLK240_o                             : out std_logic; 
    GBT_RX_CLK240_EN_o                          : out std_logic_vector (1 to NUM_LINKS); 
    GBT_RX_WORDCLK_o                            : out std_logic_vector (1 to NUM_LINKS);
    ...
    -- 
    DEBUG_CLK_ALIGNMENT_debug                   : in gbt_reg3_A       (1 to NUM_LINKS);
    ...
    --== BER 
    countWordReceived_o                            :out  gbt_reg32_A(1 to NUM_LINKS);
    countBitsModified_o                            :out  gbt_reg32_A(1 to NUM_LINKS);
    countWordErrors_o                              :out  gbt_reg32_A(1 to NUM_LINKS);
    --==  Control 
...
architecture structural of gbt_top is
   ...
   signal countWordReceived_s                        : gbt_reg32_A(1 to NUM_LINKS)  := (others=> (others => '0'));
   signal countBitsModified_s                        : gbt_reg32_A(1 to NUM_LINKS)  := (others=> (others => '0'));
   signal countWordErrors_s                          : gbt_reg32_A(1 to NUM_LINKS)  := (others=> (others => '0'));
   ...
   signal modifiedBitsCnt                : gbt_reg8_A (1 to NUM_LINKS);
   signal gbtModifiedBitFlagFiltered     : gbt_reg128_A(1 to NUM_LINKS);
   signal gbtModifiedBitFlag             :gbt_reg84_A(1 to NUM_LINKS) := (others => (others => '0')); 
   ...
   -- ==   FUNCTION        ==
   -- =======================
       --  Comment:  Purpose:  General Reset  (Only one Reset), 
       function  gen_rst_common_func ( gen_rst_from_ipbus_slaves: in std_logic_vector) 
       return  std_logic_vector is 
         variable rst_o:  std_logic_vector (1 to NUM_LINKS) := (others => '0');
         variable value:  integer range 0 to  64 := 0; 
           begin 
             value :=  to_integer(unsigned(gen_rst_from_ipbus_slaves));
             if  value >= 1  then  
              rst_o  := (others => '1'); 
              else  
              rst_o := (others => '0'); 
              ...
              return rst_o; 
      end function gen_rst_common_func; 

signal  gen_rst_common_s: std_logic_vector (1 to NUM_LINKS) := (others => '0'); 

-- =========================
-- ==  FUNCTION           ==
-- =========================
-- Comment. Purpose:  BER word downscaler 
    function  BER_rate_down_func ( WordReceived: in std_logic_vector)
         return  std_logic is  
          variable  count_word_recieve_down: integer range 0 to 2147483647 := 0; 
          variable                   divide: integer range 1 to 6 := 6; 
          variable                    res   : integer range 0 to 2147483647 := 0; 
          variable  count_word_func   : std_logic := '0'; 
            begin 
               count_word_recieve_down := to_integer(unsigned(WordReceived)); 
                res := count_word_recieve_down rem divide;
                if  res  = 0  then 
                   count_word_func :=  '1';
                else  
                   count_word_func  := '0'; 
                 end if ; 
                return count_word_func; 
         end function  BER_rate_down_func;
       
signal  count_word_recived_bc_s:  gbt_reg32_A(1 to NUM_LINKS)  := (others=> (others => '0'));    
signal  count_active_func_s    :  std_logic_vector (1 to NUM_LINKS) := (others => '0');        
   --=====================================================================================--  
--=================================================================================================--
...
--=================================================================================================--
  ...
  gen_rst_common_s <= gen_rst_common_func(CPU_RESET); 
   --===============--
   -- General reset -- 
   --===============--
 gen_Rst_gen: for i in 1 to NUM_LINKS generate  
   genRst: entity work.xlx_ku_reset
      ...
      port map (     
         ...
         RESET1_B_I                                  => not gen_rst_common_s(i), 
...
gbtExmplDsgn_inst: entity work.xlx_ku_gbt_example_design
     ...
     port map (
       ...
       GBTBANK_GBT_DATA_O                                      => gbt_data_user_o,
       ...
       -- ===============================
       --  Comment:  Must be sync with TTC_PON data.         
       GBT_TX_CLK240_i                                          => GBT_TX_CLK240_i, 
       GBT_TX_CLK240_EN_i                                       => GBT_TX_CLK240_EN_i,                     -- Comment: OLT Strobe  TX
		
       GBT_RX_CLK240_o                                          => GBT_RX_CLK240_o, 
       GBT_RX_CLK240_EN_o                                       => GBT_RX_CLK240_EN_o,                     -- Comment:  
...
--end generate; 
   ...
   -- BER                                 --
   --=====================================--
BER_gen: for i in 1 to NUM_LINKS  generate 
countWordReceivedProc: PROCESS(reset_from_genRst(i), rxframeclk_from_gbtExmplDsgn(i))
 begin

   if reset_from_genRst(i) = '1' then
       countWordReceived_s(i) <= (others => '0');
       countBitsModified_s(i) <= (others => '0');
       countWordErrors_s(i)   <= (others => '0');
       
   elsif rising_edge(rxframeclk_from_gbtExmplDsgn(i)) then
       if gbtRxReady_from_gbtExmplDsgn(i) = '1' then
           
           if gbtErrorDetected(i) = '1' then
               countWordErrors_s(i)    <= std_logic_vector(unsigned(countWordErrors_s(i)) + 1 );                
           end if;
           
          countWordReceived_s(i) <= std_logic_vector(unsigned(countWordReceived_s(i)) + 1 );
          count_active_func_s(i) <= BER_rate_down_func(countWordReceived_s(i));
           
       end if;
       
       countBitsModified_s(i) <= std_logic_vector(unsigned(modifiedBitsCnt(i)) + unsigned(countBitsModified_s(i)) );
   end if;
end process;
end generate; 

BER_gen_bc: for i in 1 to NUM_LINKS  generate
Count_word_receive_bc_proc: PROCESS ( rxframeclk_from_gbtExmplDsgn(i))
    begin 
       if rising_edge ( rxframeclk_from_gbtExmplDsgn(i)) then 
         if reset_from_genRst(i) = '1' then 
              count_word_recived_bc_s (i)  <= (others => '0');
        else  
          if count_active_func_s (i) = '1'  then 
            count_word_recived_bc_s (i) <= std_logic_vector(unsigned(count_word_recived_bc_s(i)) + 1 );
          else 
          count_word_recived_bc_s (i) <=  count_word_recived_bc_s (i); 
         end if; 
        end if;
     end if; 
 end process; 
...
countWordReceived_o   <= count_word_recived_bc_s;
countBitsModified_o   <= countBitsModified_s;
countWordErrors_o     <= countWordErrors_s;


 gbtModified_gen: for i in 1 to NUM_LINKS  generate  
   gbtModifiedBitFlagFiltered(i)(127 downto 84) <= (others => '0');
   gbtModifiedBitFlagFiltered(i)(83 downto 0) <= gbtModifiedBitFlag (i) when gbtRxReady_from_gbtExmplDsgn(i) = '1' else
                                             (others => '0');
  end generate;
---==================================================
 ---===         Comment:  CountOnes                ===
 ---==================================================   
countOnes_gen: for  i in 1 to NUM_LINKS  generate 
   countOnesCorrected: entity work.CountOnes
   Generic map (SIZE           => 128,
                MAXOUTWIDTH    => 8
    )
    Port map( 
            clk_i    => rxframeclk_from_gbtExmplDsgn(i), -- Warning: Because the enable signal (1 over 3 or 6 clock cycle) is not used, the number of error is multiplied by 3 or 6.
            I        => gbtModifiedBitFlagFiltered(i),
            O        => modifiedBitsCnt(i));            
 end generate; 
     -- ==============================  
    -- ==  RESET FROM THE USER    ===
   ...
   gral_gen: for i in 1 to NUM_LINKS generate 
       generalReset_from_user(i)  <=  ipb_rst;
  end generate;
  --
     -- Comment: Added by LPM 
   ...
   GBT_TX_WORDCLK_o <= txWordClk_from_gbtExmplDsgn;
   GBT_RX_WORDCLK_o <= rxWordClk_from_gbtExmplDsgn;
