...
entity xlx_ku_gbt_example_design is
	generic (
		...
		DATA_GENERATOR_ENABLE							: integer range 0 to 1 := 0;   -- Comment: By LPM 
		DATA_CHECKER_ENABLE								: integer range 0 to 1 := 0;   -- Comment: By LPM 
		MATCH_FLAG_ENABLE								: integer range 0 to 1 := 1;
		CLOCKING_SCHEME                                 : integer range 0 to 1 := FULL_MGTFREQ--BC_CLOCK
	);
   port ( 
		...
		-- Reset        --
		--==============--
		GBTBANK_GENERAL_RESET_I							: in  std_logic_vector(1 to NUM_LINKS);
		GBTBANK_MANUAL_RESET_TX_I						: in  std_logic_vector(1 to NUM_LINKS);
		GBTBANK_MANUAL_RESET_RX_I						: in  std_logic_vector(1 to NUM_LINKS);
        ...
        GBT_TX_CLK240_i                            : in std_logic;   -- Comment. 240 Mhz CLK 
        GBT_TX_CLK240_EN_i                         : in std_logic_vector (1 to NUM_LINKS);     -- Comment. Tick, sync w.r.t  TTC_PON CLOCK 
		
		GBT_RX_CLK240_o                            : out std_logic;   -- Comment. 240 Mhz CLK 
        GBT_RX_CLK240_EN_o                         : out std_logic_vector (1 to NUM_LINKS);     -- Comment. Tick, sync w.r.t  TTC_PON CLOCK
		...
		GBTBANK_RXFRAMECLK_ALIGNPATTER_I                : in gbt_reg3_A(1 to NUM_LINKS);	
		...
		GBTBANK_TX_MATCHFLAG_O							: out std_logic_vector(1 to NUM_LINKS);
		...
		-- XCVR ctrl    --
		--==============--
		GBTBANK_LOOPBACK_I								 : in gbt_devspec_reg3_A(1 to NUM_LINKS);
...
architecture structural of xlx_ku_gbt_example_design is  
   ...
   --================================ Signal Declarations ================================--
   -- ===  CTP  ALICE  signal  ==== --- 
   signal  TX_FRAMECLK_CTP_s             :  std_logic_vector (1 to NUM_LINKS); 
...
--=================================================================================================--
   -- ALICE CTP DECLARATION. 
   TX_FRAMECLK_CTP_s  <=  mgt_txwordclk_s;
    ...
    gbtBank_Clk_gen: for i in 1 to NUM_LINKS generate
        ...
        gbtBank_rxFrmClkPhAlgnr: entity work.gbt_rx_frameclk_phalgnr
            ...
            port map (            
                ...
                CLK_ALIGN_CONFIG                          => GBTBANK_RXFRAMECLK_ALIGNPATTER_I(i),
            ...
            RX_FRAMECLK_O(i)  <= TX_FRAMECLK_CTP_s(i);
            TX_FRAMECLK_O(i)  <= TX_FRAMECLK_CTP_s(i);
     ...
     gbtBank_rst_gen: for i in 1 to NUM_LINKS generate
         ...
         gbtBank_gbtBankRst: entity work.gbt_bank_reset    
             generic map (
                 INITIAL_DELAY                          =>  (1 * 40e6)*6   --          * 1s
             )
             port map (
                 GBT_CLK_I                              => FRAMECLK_40MHZ,
                 TX_FRAMECLK_I                          => TX_FRAMECLK_CTP_s(i) , -- Comment: Modified by LPM gbt_txframeclk_s(i) ,
                 TX_CLKEN_I                             => GBT_TX_CLK240_EN_i(i), -- Comment: Modified by LPM gbt_txclken_s(i),
                 ...
                 -- Resets scheme --  
                 --===============--  
                 GENERAL_RESET_I                        => GBTBANK_GENERAL_RESET_I(i),
                 TX_RESET_I                             => GBTBANK_MANUAL_RESET_TX_I(i),
                 RX_RESET_I                             => GBTBANK_MANUAL_RESET_RX_I(i),
           ...
           GBT_RX_CLK240_EN_o(i) <= gbt_rxclkenLogic_s(i);
     end generate;
   ...
   dataGenEn_gen: if DATA_GENERATOR_ENABLE = ENABLED generate
       ...
       dataGenEn_output_gen: for i in 1 to NUM_LINKS generate    
             ...
             port map (
                    GENERAL_RST_I                                  => GBTBANK_GENERAL_RESET_I(i) or GBTBANK_MANUAL_RESET_TX_I(i),
   ...
   dataCheckEn_gen: if DATA_CHECKER_ENABLE = ENABLED generate
       ...
       gbtBank_patCheck_gen: for i in 1 to NUM_LINKS generate
           gbtBank_pattCheck: entity work.gbt_pattern_checker
               port map (
                   RESET_I                                        => GBTBANK_GENERAL_RESET_I(i) or GBTBANK_MANUAL_RESET_RX_I(i),--gbt_rxreset_s(i),         
                   RX_FRAMECLK_I                                  => gbt_rxframeclk_s(i),
                         RX_CLKEN_I                               => gbt_rxclkenLogic_s(i), 
                   -----------------------------------------------           
   ...
   gbtBank_mgt_gen: for i in 1 to NUM_LINKS generate
       ...
       mgt_devspecific_to_s.loopBack(i)           <= GBTBANK_LOOPBACK_I(i);
   ...
   gbt_inst: entity work.gbt_bank
     ...
     port map(   
       ...
       --========--
       MGT_CLK_i                => XCVRCLK,
       GBT_TXFRAMECLK_i         => TX_FRAMECLK_CTP_s,
       GBT_TXCLKEn_i            => GBT_TX_CLK240_EN_i,    -- Comment: Modified by LPM gbt_txclken_s,
       ...
       GBT_ISDATAGLAG_o         => GBTBANK_RX_ISDATA_SEL_O,
  ...
  --============--
   matchFlag_gen: if MATCH_FLAG_ENABLE = ENABLED generate
    ...
    gbtBank_txFlag_gen: for i in 1 to NUM_LINKS generate
           gbtBank_txFlag: entity work.gbt_pattern_matchflag
               PORT MAP (
                   RESET_I                                           => gbt_txreset_s(i),
                   CLK_I                                             => TX_FRAMECLK_CTP_s(i),
                   CLKEN_I                                           => GBT_TX_CLK240_EN_i(i),    -- Comment: Modified by LPM gbt_txclken_s(1),
                   DATA_I                                            => gbt_txdata_s(i),
                   MATCHFLAG_O                                       => GBTBANK_TX_MATCHFLAG_O(i)
               );
       end generate; 
